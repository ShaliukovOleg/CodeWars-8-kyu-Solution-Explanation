// Solution / Решение

function twoSort(s) {
    // вернем отсортированный список и возмем первый элемент разделим его и после каждого символа добавим *** с помощью метода join
    return (s.sort())[0].split('').join('***');
}

// Tests

console.log(twoSort(["bitcoin", "take", "over", "the", "world", "maybe", "who", "knows", "perhaps"])); // b***i***t***c***o***i***n
console.log(twoSort(["turns", "out", "random", "test", "cases", "are", "easier", "than", "writing", "out", "basic", "ones"])); // a***r***e

/*

You will be given a list of strings. You must sort it alphabetically 
(case-sensitive, and based on the ASCII values of the chars) 
and then return the first value.

The returned value must be a string, and have "***" 
between each of its letters.

You should not remove or add elements from/to the array.

*/

/*

Вам будет дан список строк. Необходимо отсортировать его по алфавиту 
(с учетом регистра и на основе ASCII-значений символов) 
и вернуть первое значение.

Возвращаемое значение должно быть строкой и содержать "***" 
между буквами.

Не следует удалять или добавлять элементы из массива.

*/