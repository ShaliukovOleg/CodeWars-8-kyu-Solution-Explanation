...
const binStr = (str) => // Объявление функции binStr с одним параметром str, представляющим строку из битов
    [...str].reduce((acc, elem, index) => // Используется стрелочная функция и оператор spread (...) для преобразования строки str в массив
        acc += elem ^ (str[index - 1] || '0') && +!!(elem), // Используется метод reduce для итерации по массиву битов и выполнения операций
